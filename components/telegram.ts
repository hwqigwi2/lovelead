...
interface TelegramWebApp {
  initData: string; ready(): void; expand(): void; openLink(url: string): void; openTelegramLink?(url: string): void;
...
export function isTelegramUrl(url: string) { return TELEGRAM_HOST.test(url); }
export function openSmartLink(url: string) {
  const app = telegram();
  if (!app) { window.open(url, "_blank", "noopener,noreferrer"); return; }
  if (isTelegramUrl(url) && app.openTelegramLink) app.openTelegramLink(url); else app.openLink(url);
}
