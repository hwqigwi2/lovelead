...
export function Reviews() {
  return <section className="reviews-section" aria-labelledby="reviews-title"><div className="section-heading"><span className="eyebrow">ОТЗЫВЫ НАШИХ</span><h2 id="reviews-title">Пользователей</h2></div><div className="reviews-carousel">{demoReviews.map((review) => <article key={review.name} className="review-card"><h3 className="review-name">{review.name}</h3><div className="review-stars" aria-label="Оценка 5 из 5">{stars.map((star) => <Star key={star} size={14} fill="currentColor" strokeWidth={0} />)}</div><p className="review-text">{review.text}</p></article>)}</div><button className="reviews-more" onClick={() => openSmartLink(appConfig.reviewsTelegramUrl)}><span>Больше отзывов в Telegram</span><ArrowUpRight size={17} /></button></section>;
}
