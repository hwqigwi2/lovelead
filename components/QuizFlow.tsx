...
const questions: Array<{ key: keyof QualificationInput; title: string; type: "age" | "boolean" }> = [
  { key: "age", title: "Сколько тебе лет?", type: "age" }, { key: "is_military", title: "Являешься или был военнослужащим?", type: "boolean" }, { key: "has_arrest", title: "Есть ли у тебя аресты или ограничения по банковским счетам?", type: "boolean" },
];
...
export function QuizFlow({ user, initData, onCompleted }: { user: LoveLeadUser; initData: string; onCompleted: (user: LoveLeadUser) => void }) {
  ...
  const answer = async (value: boolean | number) => {
    ...
    if (question.key === "age" && Number(value) < 18) { await submit({ ...next, is_military: false, has_arrest: false } as QualificationInput); return; }
  ...
  if (step === -1) return <main className="welcome"><div className="welcome-copy"><span className="eyebrow">LOVELEAD</span><h1>Привет, {user.first_name}!</h1><p>Пройди небольшой опрос и мы подберем наиболее подходящие для тебя задания</p></div><button className="primary-button" onClick={() => { haptic("impact"); setStep(0); }}><span>Начать</span><ArrowRight size={19} /></button></main>;
