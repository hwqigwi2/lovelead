...
export function Dashboard({ user, initData, onRestart }: { user: LoveLeadUser; initData: string; onRestart: () => void }) {
  ...
  const openTask = (task: PartnerTask) => { if (task.status === "locked") return; setSelected(task); setRkoStage(RKO_SLUGS.includes(task.slug) && !rkoDone ? "quiz" : null); };
  ...
  return <main className="dashboard">{view === "home" && <header className="app-header"><div className="app-header-user"><div className="avatar">{user.avatar_url ? <Image src={user.avatar_url} alt="" fill sizes="40px" /> : user.first_name.slice(0, 1)}</div><p>Привет, <strong>{user.first_name}</strong></p></div><Image className="app-header-logo" src="/logo.png" alt="LoveLead" width={124} height={124} /></header>}{content}<BottomNav view={view} onChange={setView} /></main>;
}
