...
export function Dashboard({ user, initData, onRestart }: { user: LoveLeadUser; initData: string; onRestart: () => void }) {
  const [tasks, setTasks] = useState<PartnerTask[]>([]); const [view, setView] = useState<NavView>("home"); const [selected, setSelected] = useState<PartnerTask | null>(null); const [rkoStage, setRkoStage] = useState<"quiz" | "guide" | null>(null); const [error, setError] = useState("");
  ...
  const openTask = (task: PartnerTask) => { setSelected(task); setRkoStage(RKO_SLUGS.includes(task.slug) ? "quiz" : null); };
  if (selected && rkoStage === "quiz") return <RkoQuiz onExit={() => { setSelected(null); setRkoStage(null); }} onCompleted={(guideRequired) => setRkoStage(guideRequired ? "guide" : null)} />;
  if (selected && rkoStage === "guide") return <RkoGuide onCompleted={() => setRkoStage(null)} />;
