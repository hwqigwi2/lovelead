  ...
  return (
    <main className="detail-page">
      ...
      <section className="detail-card">
        ...
        {isRko && (
          <>
            <div className="note-block">Если у тебя уже есть ИП — не закрывай его самостоятельно. Сначала уточни ситуацию у менеджера. Если есть НПД, сообщи об этом менеджеру.</div>
            <button className="education-toggle" onClick={() => setOpenEducation(!openEducation)}>Что такое ИП на НПД?<ChevronDown className={openEducation ? "rotate" : ""} size={18} /></button>
            {openEducation && <p className="education-text">НПД — специальный налоговый режим для физических лиц и ИП при соблюдении установленных условий. Для НПД действуют специальные ставки налога в зависимости от типа дохода. При применении НПД отсутствует обязанность по уплате фиксированных страховых взносов за себя, однако режим имеет свои условия и ограничения. Перед оформлением уточни актуальные условия у менеджера и официальных источников.</p>}
          </>
        )}
